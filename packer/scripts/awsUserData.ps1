...
$password = ConvertTo-SecureString "{{user `new_user_password`}}" -AsPlainText -Force
$newUser = New-LocalUser -Name "{{user `new_user_name`}}" -Password $password -FullName "Ansible Remote User" -Description "Ansible remote user" 
